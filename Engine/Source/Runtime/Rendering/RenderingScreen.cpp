...
RenderingScreen::RenderingScreen(RenderDevice *pRenderDevice)
    : rd(pRenderDevice)
{
...
{
    vkDestroySemaphore(vk_device, window->image_available_semaphore, VK_NULL_HANDLE);
    vkDestroySemaphore(vk_device, window->render_finished_semaphore, VK_NULL_HANDLE);
    vkDestroySwapchainKHR(vk_device, window->swap_chain, VK_NULL_HANDLE);
    vkDestroyRenderPass(vk_device, window->render_pass, VK_NULL_HANDLE);
    _clean_up_swap_chain();
    vkDestroySurfaceKHR(vk_instance, window->vk_surface, VK_NULL_HANDLE);
    free(window);
}

void RenderingScreen::Initialize(Window *vScreenWindow)
{
    VkResult U_ASSERT_ONLY err;

    /* imalloc display window struct and set surface */
    window = (_Window *) imalloc(sizeof(_Window));
    screenWindow = vScreenWindow;
    screenWindow->CreateWindowSurfaceKHR(vk_instance, VK_NULL_HANDLE, &window->vk_surface);
    ...
    err = vkCreateSemaphore(vk_device, &semaphore_create_info, VK_NULL_HANDLE, &window->image_available_semaphore);
    assert(!err);

    err = vkCreateSemaphore(vk_device, &semaphore_create_info, VK_NULL_HANDLE, &window->render_finished_semaphore);
    assert(!err);

    _create_swap_chain();
}

void RenderingScreen::CmdBeginScreenRender(VkCommandBuffer *pCmdBuffer)
{
    _update_swap_chain();
    ...
    rd->CmdBufferBegin(cmd_buffer, VK_COMMAND_BUFFER_USAGE_RENDER_PASS_CONTINUE_BIT);
    ...
    rd->CmdBeginRenderPass(cmd_buffer, window->render_pass, 1, &clearColor, window->swap_chain_resources[acquire_next_index].framebuffer, &rect);

    *pCmdBuffer = cmd_buffer;
}

void RenderingScreen::CmdEndScreenRender(VkCommandBuffer cmdBuffer)
{
...
void RenderingScreen::_create_swap_chain()
{
    ...
    if (!old_swap_chain) {
        ...
        err = vkCreateRenderPass(vk_device, &render_pass_create_info, VK_NULL_HANDLE, &window->render_pass);
        assert(!err);
    }
    ...
    err = vkCreateSwapchainKHR(vk_device, &swap_chain_create_info, VK_NULL_HANDLE, &window->swap_chain);
    assert(!err);

    if (old_swap_chain)
        vkDestroySwapchainKHR(vk_device, old_swap_chain, VK_NULL_HANDLE);
    ...
    for (uint32_t i = 0; i < window->image_buffer_count; i++) {
        ...
        err = vkCreateImageView(vk_device, &image_view_create_info, VK_NULL_HANDLE, &(window->swap_chain_resources[i].image_view));
        assert(!err);
        ...
        err = vkCreateFramebuffer(vk_device, &framebuffer_create_info, VK_NULL_HANDLE, &(window->swap_chain_resources[i].framebuffer));
        assert(!err);
    }
}
...
void RenderingScreen::_clean_up_swap_chain()
{
    for (uint32_t i = 0; i < window->image_buffer_count; i++) {
        ...
        vkDestroyFramebuffer(vk_device, window->swap_chain_resources[i].framebuffer, VK_NULL_HANDLE);
        vkDestroyImageView(vk_device, window->swap_chain_resources[i].image_view, VK_NULL_HANDLE);
    }
